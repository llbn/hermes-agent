...
async def send_telegram_text(
    ...
    """Send a Telegram text message using Telegram HTML rendering semantics."""
    from gateway.platforms.telegram import ParseMode, check_telegram_requirements
    ...
    try:
        chunks = markdown_to_telegram_chunks(content, limit=DEFAULT_TELEGRAM_TEXT_LIMIT)
        message_ids = []
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                error_text = str(html_error).lower()
                if "parse" in error_text or "entity" in error_text or "html" in error_text:
