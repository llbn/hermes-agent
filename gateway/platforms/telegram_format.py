...
class TelegramFormattedChunk:
    """A single Telegram-ready message chunk."""
...
def _markdown_parser() -> MarkdownIt:
    """Return the configured markdown parser for Telegram output."""
    return MarkdownIt(
        "default",
        {
            "html": False,
...
def markdown_to_telegram_html(markdown: str | None) -> str | None:
    """Render markdown-like text to Telegram-safe HTML."""
    if not markdown:
        return markdown
...
def markdown_to_telegram_chunks(
    ...
    """Render markdown-like text to Telegram HTML chunks plus plain fallbacks."""
...
def telegram_html_to_plain_text(rendered_html: str | None) -> str | None:
    """Convert Telegram-safe HTML back into readable plain text."""
...
def _find_html_entity_end(text: str, start: int) -> int:
    ...
    if text[index] == "#":
        ...
        if text[index] in ("x", "X"):
            index += 1
            hex_start = index
            while index < len(text) and re.match(r"[0-9A-Fa-f]", text[index]):
                index += 1
            if index == hex_start:
                return -1
        else:
        ...
        name_start = index
        while index < len(text) and re.match(r"[A-Za-z0-9]", text[index]):
            index += 1
        if index == name_start:
            return -1
...
def _wrap_file_references(text: str) -> str:
    ...
    for match in _FILE_REFERENCE_RE.finditer(text):
        ...
        if filename.startswith("//"):
            continue
        if prefix.lower().endswith("http://") or prefix.lower().endswith("https://"):
            continue
        if _looks_like_domain_path(filename):
            continue
...
def _looks_like_domain_path(value: str) -> bool:
    ...
    if len(segments) <= 1:
        return False
    return any("." in segment for segment in segments[:-1])


def _is_file_reference(href: str, label: str) -> bool:
    stripped = re.sub(r"^https?://", "", href, flags=re.IGNORECASE)
    if stripped != label:
        return False
    ...
    extension = label[dot_index + 1 :].lower()
    if extension not in _FILE_EXTENSIONS_WITH_TLD:
        return False

    return not _looks_like_domain_path(label)
